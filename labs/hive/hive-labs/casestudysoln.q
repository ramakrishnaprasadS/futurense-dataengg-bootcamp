...
#Load Data
LOAD DATA LOCAL INPATH '/home/ram/futurense_hadoop-pyspark/labs/dataset/bankmarket/bankmarketdata.csv' OVERWRITE INTO TABLE marketing_data;
...
#1 Marketing Success Rate
    ...
    select 
    round((avg(if(y='yes',1,0))*100,2) as success_rate
    from marketing_data_view;
...
#3 Max,Min, Mean age of targeted customer
    select min(age) as min_age, max(age) as max_age, avg(age) as avg_age,percentile_approx(age,0.5) as med_age from marketing_data_view;
